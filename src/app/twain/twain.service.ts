...
import { concatWith, map, retry, switchMap, take } from 'rxjs/operators';
...
export class TwainService {
  ...
  getQuote(): Observable<string> {
    return new Observable((observer: Observer<number>) => observer.next(this.nextId++)).pipe(
      ...
      retry({delay:errors => errors.pipe(
        ...
        concatWith(throwError(()=>new Error('There are no Twain quotes'))) // didn't find id:1
      )})
    );
  }
}
